# AES-128 (FIPS-197) + CBC mode + PKCS#7 padding

import hashlib

# AES parameters
BLOCK_SIZE = 16   # AES block size (bytes)
Nb = 4      # number of columns in AES state
Nk = 4     # key length in 32 bit words (AES-128)
Nr = 10    # number of AES rounds

# ---------------- XOR + PKCS7 ----------------

def xor_bytes(a, b):
    # XOR two equal length byte arrays (used in CBC mode)
    if len(a) != len(b):
        raise ValueError("length mismatch")
    return bytes(x ^ y for x, y in zip(a, b))

def pkcs7_pad(data, block_size=16):
    # add PKCS#7 padding to make data a multiple of block size
    pad_len = block_size - (len(data) % block_size)
    return data + bytes([pad_len]) * pad_len

def pkcs7_unpad(data, block_size=16):
    # remove and validate PKCS#7 padding
    pad_len = data[-1]
    if pad_len <= 0 or pad_len > block_size:
        raise ValueError("bad padding")
    if data[-pad_len:] != bytes([pad_len]) * pad_len:
        raise ValueError("bad padding")
    return data[:-pad_len]

# ---------------- AES tables ----------------

# Substitution box used in SubBytes step
SBOX = [
    
]

# round constants for key expansion
RCON = [0x00,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1B,0x36]

# ---------------- AES core ----------------

def xtime(a):
    # multiply by x in GF(2^8)
    a <<= 1
    if a & 0x100:
        a ^= 0x11B
    return a & 0xFF

# Galois field multiplication
def gf_mul(a, b):
    res = 0
    for _ in range(8):
        if b & 1:
            res ^= a
        a = xtime(a)
        b >>= 1
    return res & 0xFF

def bytes_to_state(block):
    # Convert 16-byte block into AES 4x4 state matrix
    return [[block[r + 4*c] for c in range(4)] for r in range(4)]

def state_to_bytes(state):
    # Convert AES state matrix back to 16-byte block
    out = bytearray(16)
    for c in range(4):
        for r in range(4):
            out[r + 4*c] = state[r][c]
    return bytes(out)

def add_round_key(state, rk):
    # XOR state with round key
    for c in range(4):
        for r in range(4):
            state[r][c] ^= rk[r + 4*c]

# substitute bytes using S-box
def sub_bytes(state):
    for r in range(4):
        for c in range(4):
            state[r][c] = SBOX[state[r][c]]

def shift_rows(state):
    state[1] = state[1][1:] + state[1][:1]
    state[2] = state[2][2:] + state[2][:2]
    state[3] = state[3][3:] + state[3][:3]

def mix_columns(state):
    # MixColumns transformation
    for c in range(4):
        a0,a1,a2,a3 = state[0][c],state[1][c],state[2][c],state[3][c]
        state[0][c] = gf_mul(a0,2)^gf_mul(a1,3)^a2^a3
        state[1][c] = a0^gf_mul(a1,2)^gf_mul(a2,3)^a3
        state[2][c] = a0^a1^gf_mul(a2,2)^gf_mul(a3,3)
        state[3][c] = gf_mul(a0,3)^a1^a2^gf_mul(a3,2)

# expand AES key into round keys
def key_expansion(key):
    w = [list(key[4*i:4*i+4]) for i in range(4)]
    for i in range(4, 44):
        temp = w[i-1][:]
        if i % 4 == 0:
            temp = temp[1:] + temp[:1]
            temp = [SBOX[x] for x in temp]
            temp[0] ^= RCON[i//4]
        w.append([w[i-4][j] ^ temp[j] for j in range(4)])

    rks = []
    for r in range(11):
        rk = bytearray(16)
        for c in range(4):
            rk[4*c:4*c+4] = bytes(w[r*4 + c])
        rks.append(bytes(rk))
    return rks

# Encrypt a single 16-byte block using AES
def aes_encrypt_block(block, rks):
    s = bytes_to_state(block)
    add_round_key(s, rks[0])
    for r in range(1, 10):
        sub_bytes(s)
        shift_rows(s)
        mix_columns(s)
        add_round_key(s, rks[r])
    sub_bytes(s)
    shift_rows(s)
    add_round_key(s, rks[10])
    return state_to_bytes(s)

def inv_sub_bytes(state):
    inv = [0]*256
    for i in range(256):
        inv[SBOX[i]] = i
    for r in range(4):
        for c in range(4):
            state[r][c] = inv[state[r][c]]

def inv_shift_rows(state):
    state[1] = state[1][-1:] + state[1][:-1]
    state[2] = state[2][-2:] + state[2][:-2]
    state[3] = state[3][-3:] + state[3][:-3]

def inv_mix_columns(state):
    for c in range(4):
        a0,a1,a2,a3 = state[0][c],state[1][c],state[2][c],state[3][c]
        state[0][c] = gf_mul(a0,14)^gf_mul(a1,11)^gf_mul(a2,13)^gf_mul(a3,9)
        state[1][c] = gf_mul(a0,9)^gf_mul(a1,14)^gf_mul(a2,11)^gf_mul(a3,13)
        state[2][c] = gf_mul(a0,13)^gf_mul(a1,9)^gf_mul(a2,14)^gf_mul(a3,11)
        state[3][c] = gf_mul(a0,11)^gf_mul(a1,13)^gf_mul(a2,9)^gf_mul(a3,14)

# Decrypt a single AES block
def aes_decrypt_block(block, rks):
    s = bytes_to_state(block)
    add_round_key(s, rks[10])
    for r in range(9, 0, -1):
        inv_shift_rows(s)
        inv_sub_bytes(s)
        add_round_key(s, rks[r])
        inv_mix_columns(s)
    inv_shift_rows(s)
    inv_sub_bytes(s)
    add_round_key(s, rks[0])
    return state_to_bytes(s)

# AES-CBC encryption
def cbc_encrypt(msg, key, iv):
    padded = pkcs7_pad(msg)
    round_keys = key_expansion(key)   # expanded AES round keys

    ciphertext = b""
    prev_block = iv

    for i in range(0, len(padded), 16):
        block = padded[i:i+16]
        block = xor_bytes(block, prev_block)  # CBC XOR

        # AES block encryption
        encrypted = aes_encrypt_block(block, round_keys)

        ciphertext += encrypted
        prev_block = encrypted

    return ciphertext


def cbc_decrypt(ciphertext, key, iv):
    rks = key_expansion(key)
    out = b""
    prev = iv
    for i in range(0, len(ciphertext), 16):
        block = ciphertext[i:i+16]
        p = aes_decrypt_block(block, rks)
        out += xor_bytes(p, prev)
        prev = block
    return pkcs7_unpad(out)

# ---------------- KEY + IV ----------------

def derive_key(student_id):
    # derive AES-128 key from student ID using MD5
    return hashlib.md5(student_id.encode()).digest()

def derive_reference_iv(student_id):
    # base IV from SHA-256(student_id)
    return hashlib.sha256(student_id.encode()).digest()[:16]

def iv_from_reference(ref_iv, counter):
    # converting base IV from bytes to 128bit integer
    x = int.from_bytes(ref_iv, "big")

    # incrementing IV using counter for randomness (wraps at 2^128)
    x = (x + counter) % (1 << 128)

    # convert back to 16 byte IV
    return x.to_bytes(16, "big")

